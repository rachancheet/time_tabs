    ...
    <form onSubmit={handlesubmit}>      
            ...
            <div className='flex flex-col my-4'> 
          {
            dayview.map((nm,index2)=>(
              <div className='my-5'>
                      {timings[index2]}: <input className='rounded' onChange={(e)=>{
                        let copy = fact;
                        ...
                        setfact(copy); 
                      }} placeholder={" "+nm} name={index2}/>             
                      </div>
                       ))
                      }
                      </div>    
              <button className='my-8 py-2 px-4 shadow-md no-underline rounded-full bg-[#16a34a] text-white font-sans font-semibold text-sm border-blue btn-primary hover:text-white hover:bg-blue-light focus:outline-none active:shadow-none mr-2'>submit</button>
    </form>
</div>
